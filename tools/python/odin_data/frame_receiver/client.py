...
class OdinDataClient(object):
    ...
    def _parse_arguments(self, prog_name=sys.argv[0]):
        ...
        parser.add_argument('--proc', type=argparse.FileType('r'), dest='proc_file', nargs='?',
                            default=None, const=sys.stdin,
                            help='Specify JSON configuration file to send as configure command')
    ...
    def run(self):
        ...
        if self.args.proc_file is not None:
            self.do_proc_cmd(self.args.proc_file)
    ...
    def do_config_cmd(self, config_file):
        ...
        try:
            ...
            print("Config params: {}".format(config_params))
    ...
    def do_proc_cmd(self, proc_file):
        """Send a proc command to odin-data with the specified JSON configuration file."""
        try:
            proc_params = json.load(proc_file)
            print("Proc params: {}".format(proc_params))

            for entry in proc_params:
                proc_msg = IpcMessage('cmd', 'configure', id=self._next_msg_id())
                for param, value in entry.items():
                    proc_msg.set_param(param, value)

                self.logger.info(
                    "Sending proc command to the odin-data application with specified parameters"
                )
                self.ctrl_channel.send(proc_msg.encode())
                self.await_response()

        except JSONDecodeError as e:
            self.logger.error("Failed to parse proc file: {}".format(e))
